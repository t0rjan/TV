...
$(function ($) {
    $.post('/anonymous/user/oper', {'_token': token, 'oper': 'getUserType'}, function (result) {
        var userTypeTmp = new Object();
        for (var i = 0; i < result.data.length; i++) {
            userTypeTmp[result.data[i].user_type] = result.data[i].type_name;
        }
        //userTypeTmp.toString().split(',');

        //window.document.getElementById('user-grid-userType').setAttribute('data-usertype',JSON.stringify(userTypeTmp));
        window.document.getElementById('user-grid-userType').setAttribute('data-usertype', userTypeTmp);

        //$('#user-grid-userType').attr('data-usertype',JSON.stringify(userTypeTmp));
        console.debug(userTypeTmp);
    });

    console.debug($('#user-grid-userType').data('usertype'));
    ...
    $(grid_selector).jqGrid({
        ...
        height: 550,
        colNames: [' ', 'ID', '用户名', '昵称', '手机', '用户类型', '创建时间', '更新时间', '状态'],
        colModel: [
            {
        ...
        editurl: "/anonymous/user/oper",//nothing is saved
        multiselect: true,
        caption: "用户列表",
        autowidth: true
